...
/// Plod implementation helper for generic Vec
pub fn vec_read_from_item_count<E: Endianness, T: Plod<E>, R: Read>(size: usize, from: &mut R) -> Result<Vec<T>> {
    let mut vec = Vec::new();
    for _ in 0..size {
...
/// Plod implementation helper for generic Vec
pub fn vec_read_from_byte_count<E: Endianness, T: Plod<E>, R: Read>(mut size: usize, from: &mut R) -> Result<Vec<T>> {
    let mut vec = Vec::new();
    while size > 0 {
